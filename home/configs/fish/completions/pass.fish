...
#
# Extended with pass-otp completions.
...
# --- pass otp completions ---

complete -c $PROG -f -n __fish_pass_needs_command -a otp -d 'Command: generate OTP code'

# OTP subcommands
complete -c $PROG -f -n '__fish_pass_uses_command otp; and not __fish_seen_subcommand_from insert append uri validate code show help version' \
    -a insert -d 'Insert a new OTP secret'
complete -c $PROG -f -n '__fish_pass_uses_command otp; and not __fish_seen_subcommand_from insert append uri validate code show help version' \
    -a append -d 'Append OTP secret to existing passfile'
complete -c $PROG -f -n '__fish_pass_uses_command otp; and not __fish_seen_subcommand_from insert append uri validate code show help version' \
    -a uri -d 'Display the key URI'
complete -c $PROG -f -n '__fish_pass_uses_command otp; and not __fish_seen_subcommand_from insert append uri validate code show help version' \
    -a validate -d 'Validate an OTP key URI'
complete -c $PROG -f -n '__fish_pass_uses_command otp; and not __fish_seen_subcommand_from insert append uri validate code show help version' \
    -a code -d 'Generate an OTP code'

# OTP entries (for `pass otp` with no subcommand = code)
complete -c $PROG -f -n '__fish_pass_uses_command otp; and not __fish_seen_subcommand_from insert append uri validate code show help version' \
    -a "(__fish_pass_print_entries)"

# OTP code options
complete -c $PROG -f -n '__fish_pass_uses_command otp; and __fish_seen_subcommand_from code show' \
    -s c -l clip -d 'Put OTP code in clipboard'
complete -c $PROG -f -n '__fish_pass_uses_command otp; and __fish_seen_subcommand_from code show' \
    -a "(__fish_pass_print_entries)"

# OTP insert / append options
for otp_sub in insert append
    complete -c $PROG -f -n "__fish_pass_uses_command otp; and __fish_seen_subcommand_from $otp_sub" \
        -s e -l echo -d 'Echo the secret on console'
    complete -c $PROG -f -n "__fish_pass_uses_command otp; and __fish_seen_subcommand_from $otp_sub" \
        -s f -l force -d 'Do not prompt before overwriting'
    complete -c $PROG -f -n "__fish_pass_uses_command otp; and __fish_seen_subcommand_from $otp_sub" \
        -s s -l secret -d 'Prompt for OTP secret instead of URI'
    complete -c $PROG -f -n "__fish_pass_uses_command otp; and __fish_seen_subcommand_from $otp_sub" \
        -s i -l issuer -d 'OTP issuer'
    complete -c $PROG -f -n "__fish_pass_uses_command otp; and __fish_seen_subcommand_from $otp_sub" \
        -s a -l account -d 'OTP account name'
    complete -c $PROG -f -n "__fish_pass_uses_command otp; and __fish_seen_subcommand_from $otp_sub" \
        -a "(__fish_pass_print_entries)"
end

# OTP uri options
complete -c $PROG -f -n '__fish_pass_uses_command otp; and __fish_seen_subcommand_from uri' \
    -s c -l clip -d 'Put key URI in clipboard'
complete -c $PROG -f -n '__fish_pass_uses_command otp; and __fish_seen_subcommand_from uri' \
    -s q -l qrcode -d 'Display QR code'
complete -c $PROG -f -n '__fish_pass_uses_command otp; and __fish_seen_subcommand_from uri' \
    -a "(__fish_pass_print_entries)"

# OTP validate
complete -c $PROG -f -n '__fish_pass_uses_command otp; and __fish_seen_subcommand_from validate' \
    -a "(__fish_pass_print_entries)"
