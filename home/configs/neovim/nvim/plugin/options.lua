...
vim.iter({
	...
	-- cmdheight of 0 shows command line only when needed
	cmdheight = 0,
	-- Height of command line when shown (for messages, etc)
	cmdwinheight = 8,
	...
	fillchars = {
		...
		--   foldinner = " ", -- no inner fold lines (shown inside fold area) nvim 0.12 stuff
		msgsep = "─",
	},
	...
	-- [0-9\-\+\*]\+ (number/dash/plus/asterisk) [\.\)]* (optional dot/parent)
	-- \s\+ (required trailing whitespace)
	formatlistpat = [[^\s*[0-9\-\+\*]\+[\.\)]*\s\+]],
	-- r: auto-insert comment leader after <Enter>
	-- q: allow formatting comments with gq
	-- n: recognupdate_in_insert = false,ize numbered lists
	...
	-- number = true, -- testing without
	-- Show relative line numbers (distance from cursor line)
	-- relativenumber = true, -- testing without
...
vim.defer_fn(function()
	vim.diagnostic.config({
		...
		-- show source of diagnostic when there are multiple
		float = {
			source = "if_many",
		},
