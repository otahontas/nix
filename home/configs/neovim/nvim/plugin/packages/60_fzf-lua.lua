...
local function git_diff_default_branch()
	local default_branch = utils.run_cmd({ "git", "default-branch-name" }, { namespace = "fzf-lua" })
...
set("n", "<leader>res", function()
	if not fzf.config.__resume_data or not fzf.config.__resume_data.opts then
		fzf.builtin()
		return
	end
	fzf.resume()
end, { desc = "Resume last picker or fallback to generic" })
...
-- git
set("n", "<leader>gddb", "<cmd>FzfLua git_diff_default_branch<cr>", { desc = "Git diff against default branch" })
